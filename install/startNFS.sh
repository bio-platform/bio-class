...
if [[ "$MODE" == "destroy" ]];then
  ...
  if [[ -n "$PASSWORD" ]];then
    # kinit for user/root
    echo "$PASSWORD" | kinit --password-file=STDIN "$USER"@"$REALM"
    echo "$PASSWORD" | sudo kinit --password-file=STDIN "$USER"@"$REALM"
  ...
  if [[ -n "$keytab_exists" ]];then
     ...
     /usr/bin/kinit -kt /etc/krb5.keytab nfs/"$USER"@"$REALM"
     sudo /usr/bin/kinit -kt /etc/krb5.keytab nfs/"$USER"@"$REALM"
